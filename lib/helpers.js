...
function validateObjectId(id, fieldName = 'ID') {
  // Throws an HttpError rather than a bare Error so the frame reports it as a 400 on its own.
  // Callers used to sniff `error.message.includes('Invalid')` to decide between 400 and 500,
  // which put the status of a validation failure in seven catch blocks instead of here.
  if (!id || typeof id !== 'string') {
    throw new HttpError(`${fieldName} is required`, 400);
  }

  if (!ObjectId.isValid(id)) {
    throw new HttpError(`Invalid ${fieldName} format`, 400);
  }
