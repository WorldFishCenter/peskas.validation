...
async function applyDownloadPermissions(user, queryParams) {
  ...
  if (user.role === 'admin') {
    ...
    if (gaul_2 && gaul_2.trim()) {
      // Single district selection (not array)
      effectiveGaulCodes = [gaul_2.trim()];
    }
    // Empty gaul_2 → empty array → no gaul_2 filter (all districts)
  } else {
    // Regular user - filter by permissions
    const allowedGaulCodes = user.permissions?.gaul_codes || [];

    if (gaul_2 && gaul_2.trim()) {
      const requestedGaulCodes = [gaul_2.trim()];
    ...
    // Empty gaul_2 → empty array → no gaul_2 filter (all districts)
    // User explicitly selected "all districts", respect that choice
  }
