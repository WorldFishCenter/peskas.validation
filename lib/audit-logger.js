 ...
 * @param {'auth'|'admin'|'validation'|'download'} event.category
...
/**
 * Number of days an audit record is retained before MongoDB expires it.
 */
 ...
 *
 * This is the only place audit indexes are declared. `lib/db.js` used to create a
 * near-identical set on every cold connection, so which indexes existed — including whether
 * records expired at all — depended on which path had run first.
 *
...
async function ensureAuditIndexes(db) {
  try {
    ...
    await col.createIndex({ timestamp: 1 }, { expireAfterSeconds: AUDIT_RETENTION_DAYS * 24 * 3600 });
  } catch (_err) {
    // Non-fatal
  }
}
