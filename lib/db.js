...
async function connectToDatabase() {
  ...
  try {
    ...
    // Ensure audit log indexes exist (non-blocking)
    db.collection('users_audit').createIndex({ timestamp: -1 }).catch(() => {});
    db.collection('users_audit').createIndex({ username: 1, timestamp: -1 }).catch(() => {});
    db.collection('users_audit').createIndex({ category: 1, timestamp: -1 }).catch(() => {});
    // TTL index: auto-expire documents older than 90 days
    db.collection('users_audit').createIndex({ timestamp: 1 }, { expireAfterSeconds: 90 * 24 * 3600 }).catch(() => {});
